...
function mod:Fear(_, spellID)
	if self.db.profile.fear then
		...
		self:Bar(L["fear_nextbar"], 30, spellID)
		self:ScheduleEvent("fear", "BigWigs_Message", 27, L["fear_warning"], "Positive")
	end
end
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	if msg == L["engage_trigger"] then
		...
		if self.db.profile.fear then
			self:Bar(L["fear_nextbar"], 30, "Spell_Shadow_PsychicScream")
			self:ScheduleEvent("fear", "BigWigs_Message", 27, L["fear_warning"], "Positive")
		end
	elseif self.db.profile.phase and msg == L["airphase_trigger"] then
		...
		self:Bar(L["landphase_message"], 54, "INV_Misc_Head_Dragon_01")
	elseif self.db.profile.phase and (msg == L["landphase_trigger1"] or msg == L["landphase_trigger2"]) then
		...
		self:Bar(L["landphase_message"], 8, "INV_Misc_Head_Dragon_01")
	end
end
