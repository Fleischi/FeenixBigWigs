...
L:RegisterTranslations("enUS", function() return {
	...
	thaladred_inc_trigger = "Let us see how your nerves hold up against the Darkener, Thaladred.",
...
function mod:OnEnable()
	...
	self:AddCombatListener("SPELL_CAST_START", "FearCast", 40636)
...
function mod:Fear(_, spellID)
	...
	if (time - last) > 5 then
		...
		if db.fear then
			...
			self:Bar(L["fear_bar"], 25, spellID)
		end
	end
end
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	if msg:find(L["engage_trigger"]) then
		self:Bar(thaladred, 30, "Spell_Shadow_Charm")
	...
	elseif msg == L["weapons_inc_trigger"] then
		...
		self:Bar(L["revive_bar"], 123, "Spell_Holy_ReviveChampion")
		self:DelayedMessage(118, L["revive_warning"], "Attention")
	...
	elseif msg == L["flying_trigger"] then
		...
		self:Bar(L["gravity_bar"], 40, "Spell_Nature_UnrelentingStorm")
...
local lastpyro = 0
function mod:Pyro(_, spellID)
	local time = GetTime()
	if (time - lastpyro) > 20 then
		lastpyro = time
		if db.pyro then
			self:Bar(L["pyro"], 60, "Spell_Fire_Fireball02")
