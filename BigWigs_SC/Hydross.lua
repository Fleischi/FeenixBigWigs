...
L:RegisterTranslations("enUS", function() return {
	...
	poison_trigger = "Aaghh, the poison...",
	poison_stance = "Hydross is now poisoned!",
	water_trigger = "Better, much better.",
...
function mod:Mark(_, spellID, _, _, spellName)
	if db.mark then
		...
		if prevPerc ~= curPerc then
			self:TriggerEvent("BigWigs_StopBar", self, fmt(debuffBar, prevPerc, poisonName))
			self:TriggerEvent("BigWigs_StopBar", self, fmt(debuffBar, prevPerc, cleanName))
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	...
	elseif msg == L["water_trigger"] then
	...
	elseif msg == L["poison_trigger"] then
