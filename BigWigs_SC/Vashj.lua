...
function mod:RepeatStrider(firsttime)
	local timer = firsttime or 60
	if db.strider then
		self:Bar(L["strider_bar"], timer, "Spell_Nature_AstralRecal")
		self:ScheduleEvent("StriderWarn", "BigWigs_Message", timer-3, L["strider_soon_message"], "Attention")
	end
	self:ScheduleEvent("Strider", self.RepeatStrider, timer, self)
end

function mod:RepeatNaga(firsttime)
	local timer = firsttime or 45
	if db.naga then
		self:Bar(L["naga_bar"], timer, "INV_Misc_MonsterHead_02")
		self:ScheduleEvent("NagaWarn", "BigWigs_Message", timer-3, L["naga_soon_message"], "Attention")
	end
	self:ScheduleEvent("Naga", self.RepeatNaga, timer, self)
end

function mod:RepeatElemental(firsttime)
	local timer = firsttime or 65
	if db.elemental then
		self:Bar(L["elemental_bar"], timer, "Spell_Nature_ElementalShields")
		delayedElementalMessage = self:DelayedMessage(timer-3, L["elemental_soon_message"], "Important")
	end
	self:ScheduleEvent("Elemental", self.RepeatElemental, timer, self)
end
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	...
	elseif db.phase and msg == L["phase3_trigger"] then
		...
		self:CancelScheduledEvent("Elemental")
		self:CancelScheduledEvent("Strider")
		self:CancelScheduledEvent("Naga")
		self:TriggerEvent("BigWigs_StopBar", self, L["elemental_bar"])
		self:TriggerEvent("BigWigs_StopBar", self, L["strider_bar"])
		self:TriggerEvent("BigWigs_StopBar", self, L["naga_bar"])
	end
end
