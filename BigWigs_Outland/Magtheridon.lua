...
L:RegisterTranslations("enUS", function() return {
	...
	escape_trigger2 = "Thank you for releasing me. Now...die!",
	...
	nova_ = "Magtheridon begins to cast blast nova.",
	nova_bar = "Next Blast Nova",
...
function mod:OnEnable()
	...
	self:AddCombatListener("SPELL_CAST_START", "Nova", 30616)
	self:AddCombatListener("SPELL_CAST_START", "Heal", 30528)
	self:AddCombatListener("SPELL_AURA_REMOVED", "BanishRemoved", 30168)
	self:AddCombatListener("SPELL_AURA_REMOVED", "StartChannelers", 30207)
...
function mod:StartChannelers()
	abycount = 1
	debwarn = nil
	self:ScheduleEvent("start", self.Start, 120, self)
	if db.escape then
...
function mod:Nova()
	if db.nova then
		...
		self:Bar(L["nova_bar"], 68, "Spell_Fire_SealOfFire")
		self:Bar(L["nova_cast"], 12, "Spell_Fire_SealOfFire")
		self:DelayedMessage(64, L["nova_warning"], "Urgent")
	end
end
