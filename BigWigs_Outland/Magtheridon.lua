...
function mod:Start()
	...
	if db.nova then
		...
		-- TODO: change the nova_bar to cast_nova_bar so only my modification is being used
	end
	if db.enrage then
...
function mod:BlastNovaCasting(_, spellID, arg2)
	...
	-- Bar with given name, 68s duration and icon
	self:Bar(L["cast_nova_bar"], 68, "Spell_Fire_SealOfFire")
	-- Bar with given name, 12s duration and icon
	self:Bar(L["cast_nova_cast"], 12, "Spell_Fire_SealOfFire")
	-- Sends a message in 60 seconds with the given text and priority
	self:DelayedMessage(60, L["cast_nova_warning"], "Urgent")
end
